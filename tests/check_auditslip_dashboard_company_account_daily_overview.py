...
save(bot_b, "B_WD_CROSS_22", "botB", "บริษัท B", "B_WD", "บริษัท B ถอน", "22/05/26", "2026-05-22", 120.0, "B-WITHDRAW-DEST", "A-WD-SOURCE", "REF09")
...
cross = next(r for r in snap["account_cross_company"] if r["account"] == "A-WD-SOURCE")
assert cross["total_amount"] == 200.0 and cross["total_count"] == 2, cross
assert {c["bot_key"]: c["amount"] for c in cross["companies"]} == {"botA": 80.0, "botB": 120.0}, cross
bot_b_cross = next(c for c in cross["companies"] if c["bot_key"] == "botB")
assert {d["date_key"]: d["amount"] for d in bot_b_cross["days"]} == {"2026-05-22": 120.0}, bot_b_cross
assert {d["date_key"]: d["amount"] for d in cross["days"]} == {"2026-05-22": 200.0}, cross
